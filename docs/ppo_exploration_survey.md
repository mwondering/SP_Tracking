...
>
> 状态：文献调研与工程建议；仓库现已支持 PPO、SAPG 与 CPO-KL/AWAC 三种
> 算法模式，CPO 的可选对抗判别器和其余推荐方案尚未实现。
...
本仓库已经实现显式可选的 PPO、SAPG 和 CPO-KL/AWAC 模式。SAPG 语义跟随作者
官方 `rl_games` 实现，CPO 在同一聚合框架上加入官方 follower PPO-KL 与 AWAC
目标，而不是只借用“多策略”名称。共享实现具有以下性质：
...
fraction、importance-ratio deviation、归一化 ESS、被选 follower 和各 block
平均标准差；CPO 还记录 follower–leader KL 与分项 loss。目前仍未直接记录：

- SAPG 模式下 leader 与每个 follower 的策略 KL；
...
该阶段的核心训练路径已经实现，并通过 `algorithm.variant: cpo` 显式选择，不会
改变 PPO 或 SAPG 的既有语义。当前配置形态为：
...
尚未实现可选 adversarial discriminator，因此当前模式对应 CPO 的 KL/AWAC 核心
路径，而不是带判别器的 full-CPO。
...
已增加算法模式和 CPO follower 温度、权重及 KL 系数；这些字段进入 checkpoint
config equality，避免旧 checkpoint 被静默按新语义加载。可选 adversarial reward
仍待实现。
...
构造阶段已经按 method 选择 SAPG/CPO，并保持 plain PPO 零行为变化。后续仍需要：
...
当前实现已增加 importance-ratio deviation、ESS 和 CPO follower–leader KL；
SAPG 自身的 follower–leader KL 与轨迹 coverage 仍应补齐。如果这些指标健康，而
SAPG/CPO 仍未改善 tracking，则问题可能不是探索不足，而是
